...
const propsGenerator = function (target, propTypes, configOptions) {
  // propTypes check
  paramsCheck(propTypes, 'React Component propTypes', ['object']); // Get and wraper generators
  ...
  Object.keys(propTypes).forEach(propName => {
    ...
    if (proptype && GENERATORS[proptype]) {
      const value = GENERATORS[proptype](propName, proptype, propTypes[propName].arg, {
        target,
        propTypes,
        configOptions
      });
